...
// --- DATA: This data is specific to the Declarations and Access Control page ---

const subPageLinks = [
    "Java - Declarations and Access Control - Section 1",
];
...
const generalQuestions = [
  {
    ...
    answer: "C",
    explanation: [
      { type: 'text', content: "Access modifiers dictate which classes, not which instances, may access features." },
      { type: 'text', content: "Methods and variables are collectively known as members. Method and variable members are given access control in exactly the same way." },
      { type: 'highlighted-text', content: "private makes a member accessible only from within its own class" },
      { type: 'highlighted-text', content: "protected makes a member accessible only to classes in the same package or subclass of the class" },
      { type: 'highlighted-text', content: "default access is very similar to protected (make sure you spot the difference) default access makes a member accessible only to classes in the same package." },
      { type: 'highlighted-text', content: "public means that all other classes regardless of the package that they belong to, can access the member (assuming the class itself is visible)" },
      { type: 'highlighted-text', content: "final makes it impossible to extend a class, when applied to a method it prevents a method from being overridden in a subclass, when applied to a variable it makes it impossible to reinitialise a variable once it has been initialised" },
      { type: 'highlighted-text', content: "abstract declares a method that has not been implemented." },
      { type: 'highlighted-text', content: "transient indicates that a variable is not part of the persistent state of an object." },
      { type: 'highlighted-text', content: "volatile indicates that a thread must reconcile its working copy of the field with the master copy every time it accesses the variable." },
      { type: 'highlighted-text', content: "After examining the above it should be obvious that the access modifier that provides the most restrictions for methods to be accessed from the subclasses of the class from another package is C - protected. A is also a contender but C is more restrictive, B would be the answer if the constraint was the \"same package\" instead of \"any package\" in other words the subclasses clause in the question eliminates default." },
    ]
  },
  {
    ...
    answer: "A",
    explanation: [
      { type: 'text', content: "Option A compiles without problem." },
      { type: 'text', content: "Option B gives error - non-static variable cannot be referenced from a static context." },
      { type: 'highlighted-text', content: "Option C package ot does not exist." },
      { type: 'text', content: "Option D gives error - non-static variable cannot be referenced from a static context." },
    ]
  },
  {
    ...
    answer: "C",
    explanation: [
      { type: 'highlighted-text', content: "(3) is correct because an abstract class doesn't have to implement any or all of its interface's methods. (4) is correct because the method is correctly implemented ((7 > 4) is a boolean)." },
      { type: 'highlighted-text', content: "(1) is incorrect because interfaces don't implement anything. (2) is incorrect because classes don't extend interfaces. (5) is incorrect because interface methods are implicitly public, so the methods being implemented must be public." },
    ]
  },
  {
    question: "Which three form part of correct array declarations?\n1. public int a [ ]\n2. static int [ ] a\n3. public [ ] int a\n4. private int a [3]\n5. private int [3] a [ ]\n6. public final int [ ] a",
    options: ["1, 3, 4", "2, 4, 5", "1, 2, 6", "2, 5, 6"],
    answer: "C",
    explanation: [
      { type: 'text', content: "(1), (2) and (6) are valid array declarations." },
      { type: 'highlighted-text', content: "Option (3) is not a correct array declaration. The compiler complains with: illegal start of type. The brackets are in the wrong place. The following would work: public int [] a" },
      { type: 'highlighted-text', content: "Option (4) is not a correct array declaration. The compiler complains with: ';' expected. A closing bracket is expected in place of the 3. The following works: private int a []" },
      { type: 'highlighted-text', content: "Option (5) is not a correct array declaration. The compiler complains with 2 errors:\n';' expected. A closing bracket is expected in place of the 3 and\n<identifier> expected A variable name is expected after a[]." },
    ]
  },
  {
    ...
    answer: "C",
    explanation: [
      { type: 'highlighted-text', content: "Option A and B are wrong because they use the default access modifier and the access modifier for the class is public (remember, the default constructor has the same access modifier as the class)." },
      { type: 'text', content: "Option D is wrong. The void makes the compiler think that this is a method specification - in fact if it were a method specification the compiler would spit it out." },
    ]
  }
];
...
const Explanation = ({ answer, explanation }: { answer: string, explanation: any[] }) => { 
    const renderHighlightedText = (text: string) => {
        const keywords = ['private', 'protected', 'default', 'public', 'final', 'abstract', 'transient', 'volatile', 'ot', 'public', 'int [] a', 'private int a []'];
...
// --- MAIN PAGE COMPONENT ---
export default function DeclarationsAndAccessControlPage() {
    ...
    <div className="bg-white min-h-screen">
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <div className="border border-gray-200 rounded-lg p-6">
          <p className="text-sm text-gray-500 mb-4">Exercise : <span className="font-semibold text-green-600">Java - Declarations and Access Control</span></p>
